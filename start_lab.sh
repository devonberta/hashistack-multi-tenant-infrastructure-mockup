...
# create vault named token configuration
...
VAULT_ADDR="http://127.0.2.10:8200" VAULT_TOKEN=`cat /vagrant/data/REGION1/AZ1/INFRA/CONTROL/VAULT/1/init.json | jq -r '.root_token'` vault write identity/oidc/role/oidc-role-R1-AZ1-INFRA-HOST-1 ttl=12h key="oidc-key-1" client_id="R1-AZ1-INFRA-CONTROL-CONSUL" template='{"consul": {"hostname": "R1-AZ1-INFRA-HOST-1" } }'
